...
def test_unified_diff_is_parsed_into_real_patch_evidence() -> None:
    ...
    assert [item["path"] for item in patch["files"]] == ["src/service.py", "tests/test_service.py"]
...
def test_required_test_receipt_and_budget_are_enforced() -> None:
    receipt = PatchIntentTwin().evaluate(_request(tests={"unit": False}, receipts={}, cost=0.80))
    assert receipt.decision is Decision.REFUSE
...
def test_expected_intent_digest_detects_silent_scope_change() -> None:
    ...
    receipt = twin.evaluate(_request(intent=changed, expected_intent_digest=original_digest))
    assert receipt.decision is Decision.REFUSE
